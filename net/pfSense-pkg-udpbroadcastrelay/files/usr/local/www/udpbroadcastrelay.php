...
if (isset($_POST['del_btn'])) {
	if (is_array($_POST['del']) && count($_POST['del'])) {
		...
		if ($need_save) {
			write_config("UDP Broadcast Relay pkg: deleted instance(s).");
			udpbr_resync();
...
$section->addInput(new Form_Checkbox(
	'enable',
	'Enable',
	'Enable the UDP Broadcast Relay service.',
...
$section->addInput(new Form_Select(
	...
	udpbr_get_carp_list()
))->setHelp('Tracks the CARP status of the selected CARP VIP. The service will only run when the selected VIP is in the MASTER state.')->setWidth(5);
$form->add($section);
