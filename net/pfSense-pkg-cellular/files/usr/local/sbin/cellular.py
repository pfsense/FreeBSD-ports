...
__version__ = "1.1.0_B22"

huawei = (lambda a: a.startswith("Huawei"))

class VoleatechLTEInterface:
    """Voleatech Serial interface for our LTE cards."""
    ...
    def __init__(self):
        ...
        search for config files ".//etc/defaults/cellular.defaults.conf" or "/etc/cellular.conf"
        """
        self.cmd_string = ""
    ...
    def init_config(self, config_file):
        ...
        if (not os.path.isfile(config_file)):
            ...
            pre_text = """\
# Default configuration for the voleatech LTE interface.
...
# If you want to introduce custom settings, you may do so in /etc/cellular.conf"""
            ...
            self.config.set("Interface", "port", "cuaU0.2")
    ...
    def widget(self, args):
        ...
        for cmd, f in cmds:
            m = re.search("{}: (\S+)"
                          .format(re.escape(cmd.split("=")[0].split("?")[0])), ret[1])
            if m:
...
if __name__ == "__main__":
    interface = VoleatechLTEInterface()
    ...
    # Argument parser
    parser = argparse.ArgumentParser(description = "Interface for LTE cards distributed by Voleatech")
