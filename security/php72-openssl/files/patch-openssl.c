...
@@ -496,6 +531,12 @@ const zend_function_entry openssl_functi
...
+
 ...
 {
 	return le_x509;
@@ -774,6 +825,18 @@ static void php_openssl_csr_free(zend_re
 }
 /* }}} */
...
@@ -6206,6 +6300,435 @@ PHP_FUNCTION(openssl_open)
 }
 /* }}} */
...
+	
+	if ((res = (struct php_x509_crl*)zend_fetch_resource2_ex(crl_res, "OpenSSL X.509 CRL", le_crl, -1)) == NULL) {
+		RETURN_FALSE;
+	}
+		
+	if(!res) {
...
+	}
+	
+	capkey = php_openssl_evp_from_zval(zcapkey, 0, capass, 0, 0, &capkeyres TSRMLS_CC);
+
+	if(!capkey) {
