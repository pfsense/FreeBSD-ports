...
if ($left != "null") {
	...
	foreach ($ds_list as $ds_key_left => $ds) {
		...
		if (!$ignore) {
			...
			if ($ds == 'offset') {
				$obj[$ds_key_out] = $obj[$ds_key_out - 1];
				$obj[$ds_key_out]['key'] = 'abs ' . $ds;
				$raw_data_abs = array();
				$data_abs = array();
				...
				$obj[$ds_key_out]['values'] = $data_abs;
				$obj[$ds_key_out]['raw'] = $raw_data_abs;
				
				if (count($stats)) {

					$stats_abs = array_map('abs', $stats);
					$obj[$ds_key_out]['min'] = min($stats_abs);
					$obj[$ds_key_out]['max'] = max($stats_abs);
					$obj[$ds_key_out]['avg'] = array_sum($stats_abs) / count($stats_abs);
