...
/usr/bin/env ASSUME_ALWAYS_YES=true /usr/sbin/pkg -c /mnt delete -f -q -y ${product}-default-config\*
cp -r /pkgs /mnt
if [ -f /mnt/boot.config ]; then /usr/bin/env ASSUME_ALWAYS_YES=true /usr/sbin/pkg -c /mnt add -fq /pkgs/${product}-default-config-serial-[0-9]*.txz; else /usr/bin/env ASSUME_ALWAYS_YES=true /usr/sbin/pkg -c /mnt add -fq /pkgs/pfSense-default-config-[0-9]*.txz; fi;
rm -rf /mnt/pkgs
...
if [ -f /usr/local/share/${product}/base.mtree ]; then /usr/sbin/mtree -U -e -q -f /usr/local/share/${product}/base.mtree -p /mnt/ > /mnt/cf/conf/mtree.log; fi;
